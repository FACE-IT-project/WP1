...
The interactive tables found below contain the meta-database for the datasets identified to contain the key drivers of change in fjord and adjacent coastal socio-ecological systems of the seven FACE-IT study sites ([D1.1](https://face-it-project.github.io/WP1/key_drivers.html)). These table may be searched for key words, and clicking at the top of each column will order the rows in ascending/descending order. Individual tables are given for each site for convenience. 
...
The interactive tables found below all contain these columns:
...
- Type: The method of data collection/creation (e.g. CTD casts or modelled data)
    ...
    - Datasets marked as 'FACE-IT' are a combination of all data from all collected sources
      - This includes the EU Arctic and Svalbard (when applicable) meta-data tables as well as PANGAEA data
...
metadata_kongsfjorden <- rbind(kong_sea_ice_inner, kong_zoo_data, kong_protist_nutrient_chla, kong_CTD_database, 
                               ...
                               kong_ferry, kong_MOSJ, kong_pavlov, kong_bischoff, kong_TWICE, kong_mooring_SAMS,
                               kong_macroalgae, kong_benthic, kong_ship_arrivals, kong_NOAA, kong_CCI, 
...
metadata_isfjorden <- rbind(is_mooring_N, is_mooring_S, is_mooring_IFO, is_mooring_GFI_N, is_mooring_GFI_S, 
                            ...
                            is_CCI, is_SOCAT, is_GLODAP, is_PG, full_product_is)
```
...
# Bind them together
metadata_stor <- rbind(ingle_cryo, ingle_CTD, ingle_plankton, stor_PAR, stor_NOAA, stor_CCI, 
...
metadata_young_sound <- rbind(young_air_pCO2, young_water_pCO2, young_CTD_bottom, young_CTD_water, 
                              ...
                              young_prim_prod, young_NOAA, young_CCI, young_PG, full_product_young)
```
...
# Bind them together
metadata_disko_bay <- rbind(disko_CTD_water, disko_CTD_ChlA, disko_NOAA, disko_CCI, disko_SOCAT, disko_GLODAP, 
...
metadata_nuup <- rbind(nuup_phyto_abundance, nuup_CTD, nuup_Chla, nuup_prim_prod, nuup_phosphate, nuup_silicate,
                       nuup_NOAA, nuup_CCI, nuup_SOCAT, nuup_PG, full_product_nuup)
```
...
metadata_porsangerfjorden <- rbind(por_mooring_GFI, por_fish_bio, por_benthos_bio, por_NorKyst, por_sea_ice, 
                                   por_hydro, por_IMR_CTD, por_IMR_plankton, por_NOAA, por_CCI, por_SOCAT, 
