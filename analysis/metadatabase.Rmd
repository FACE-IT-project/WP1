...
# Table column names
DT_cols <- c("Name", "Date", "Longitude", "Latitude", "Depth", "Files", "Link", "Reference", "Notes")
...
# Bathymetry data
EU_GEBCO <- data.frame(data_name = "Bathymetry",
                       date_range = NA,
...
EU_protists <- data.frame(data_name = "Ice cores: protist presence (%)",
...
EU_NCEI_1989 <- data.frame(data_name = "CTD: temperature, salinity",
...
EU_GRDC <- data.frame(data_name = "River discharge",
...
# Combine
metadata_EU_arctic <- rbind(EU_zooplankton, EU_ice, EU_YMER, EU_Codispoti, EU_protists, EU_NCEI_1989, EU_GRDC)
```
...
sval_glacier_area <- data.frame(data_name = "Glacier: area",
...
sval_tidal_glacier_front <- data.frame(data_name = "Glacier: tidal front",
...
sval_marine_glacier_front <- data.frame(data_name = "Glacier: marine front",
...
sval_fast <- data.frame(data_name = "Fast ice (duration)",
...
# Combine
metadata_svalbard <- rbind(sval_glacier_mass, sval_glacier_area, sval_tidal_glacier_front, sval_surface_met, sval_UNIS_database, 
                           sval_seabird_database, sval_protection, sval_NICE, sval_fast)
```
...
# Mouth mooring North
is_mooring_N <- data.frame(data_name = "Mooring: temperature, salinity, O2, velocity (U, V)",
...
Skogseth, R., & Ellingsen, P. G. (2019). Mooring data from the Isfjorden Mouth - North (I-N) during 31 Aug 2015 to 12 Aug 2016 [Data set]. Norwegian Polar Institute. https://doi.org/10.21334/npolar.2019.111aca43        Skogseth, R., & Ellingsen, P. G. (2019). Mooring data from the Isfjorden Mouth - North (I-N) during 15 Oct 2016 to 2 Oct 2017 [Data set]. Norwegian Polar Institute. https://doi.org/10.21334/npolar.2019.3078f619        
Skogseth, R., & Ellingsen, P. G. (2019). Mooring data from the Isfjorden Mouth - North (I-N) during 5 Oct 2017 to 24 Aug 2018 [Data set]. Norwegian Polar Institute. https://doi.org/10.21334/npolar.2019.e9106051\');">Skogseth et al. (2019)</a>',
                           note = NA)

# Mouth mooring South
is_mooring_S <- data.frame(data_name = "Mooring: temperature, salinity, O2, velocity (U, V)",
                           ...
                           reference = '<a onclick="alert(\'Skogseth, R., & Ellingsen, P. G. (2019). Mooring data from the Isfjorden Mouth - South (I-S) during September 2005 to September 2006 [Data set]. Norwegian Polar Institute. https://doi.org/10.21334/npolar.2019.176eea39        
...
# Mooring IFO
is_mooring_IFO <- data.frame(data_name = "Mooring: temperature, salinity, O2, velocity (U, V)",
...
is_mooring_outer <- data.frame(data_name = "Mooring: temperature, salinity, O2, velocity (U, V)",
                             date_range = "2007 to 2008",
                             lon_range = "14.41716", lat_range = "78.1383",
                             depth_range = "220",
...
# Mouth mooring GFI
is_mooring_GFI <- data.frame(data_name = "Mooring: temperature, turbidity, current (direction, speed)",
...
is_CO2_tempelfjorden <- data.frame(data_name = "CTD: temperature, salinity, TA, pH, EP TA",
...
# CO2 station at IsA
is_CO2_IsA <- data.frame(data_name = "CTD: temperature, salinity, TA, pH, EP TA",
                         date_range = "2015 to 2017",
                         lon_range = "15.5 to 15.533", lat_range = "78.257 to 78.267",
...
is_Chla_IsA <- data.frame(data_name = "CTD: chlorophyl A, phaeophytin",
...
por_fish_bio <- data.frame(data_name = "Fish species biomass",
...
# Fish species biomass
por_benthos_bio <- data.frame(data_name = "Benthos species biomass",
                              date_range = "2007 to 2016",
                              lon_range = "?", lat_range = "?",
                              depth_range = "?",
                              file_name = '<a onclick="alert(\'Not currently hosted\');">0 files</a>',
...
por_NorKyst <- data.frame(data_name = "Model: sea temperature (surface, mid, bottom), sea ice (thickness)",
