...
DT_cols <- c("Type", "Category", "Driver", "Date", "Citation", "Access", "Accessed")
...
#                             EU_SOCAT, EU_GLODAP, barents_bosa, barents_mavi, barents_seno, barents_seme, barents_gamo,
#                             barents_youngclha, EU_epagr, EU_wpagr, EU_PG)
```

```{r EU-arctic-meta-data-new}
# Extract EU and Barents Sea sites
metadata_EU_arctic <- clean_all_meta |> 
  filter(site %in% c("EU", "Barents Sea")) |> 
  ...
  mutate(pub_author = case_when(grepl("Norwegian Polar Institute", citation) ~ "NPI",
         ...
         pub_year = as.numeric(sub(".*\\(([^.]+)\\).*", "\\1", citation)),
         pub_year = case_when(is.na(pub_year) ~ "", TRUE ~ paste0(" (",pub_year,")")),
         pub_source = case_when(grepl("npolar", URL) ~ "NPI",
                                ...
                                grepl("bank.stat.gl", URL) ~ "GL Stats",
                                grepl("data.ssb", URL) ~ "Nor Stats",
                                TRUE ~ "link"),
...
DT::datatable(metadata_EU_arctic, rownames = FALSE, escape = FALSE, colnames = DT_cols,
              # filter = "top", 
              caption = "Table 1: Pan-arctic datasets. These are generally very large files. Data within these files that overlap with study sites may be found within their aggregated datasets.")
```
...
metadata_svalbard <- rbind(sval_MOSJ_glacier_mass, sval_Nature_glacier_mass, sval_glacier_area, sval_tidal_glacier_front,
                           ...
                           sval_pop, sval_tour_arrival, sval_guest_night, sval_AIS, sval_paeb, sval_odma, 
                           sval_nwmmcalanus, sval_nwtotcalanus, sval_semmcalanus, sval_setotcalanus, sval_ritr, sval_urlo,
                           sval_cycr)
```
...
metadata_kongsfjorden <- rbind(kong_sea_ice_inner, kong_sea_ice_shp, kong_zoo_data, kong_protist_nutrient_chla, kong_CTD_database, 
                               ...
                               kong_NOAA, kong_CCI, kong_MASIE,kong_lahy, kong_somobo, kong_seabirds, kong_calanus, kong_ritr, kong_urlo, 
                               kong_PG, full_product_kong)
```
...
metadata_isfjorden <- rbind(is_mooring_N, is_mooring_S, is_mooring_IFO, is_mooring_GFI_N, is_mooring_GFI_S, 
                            ...
                            is_CCI, is_MASIE, is_ritr, is_urlo, is_PG, full_product_is)
```
