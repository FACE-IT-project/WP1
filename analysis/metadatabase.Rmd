...
make_meta_data <- function(file_name, data_type, URL, reference, other = NA){
  ...
  if(str_detect(file_name, ".tsv")){
    dat <- read_tsv(file_name, progress = T, guess_max = 100000)
  } else {
  ...
  lat_min <- min(dat[,lat_col], na.rm = T)
  lat_max <- max(dat[,lat_col], na.rm = T)
  ...
  depth_col <- depth_col[!str_detect(depth_col, "Bottom")]
  depth_min <- min(dat[,depth_col], na.rm = T)
  depth_max <- max(dat[,depth_col], na.rm = T)
