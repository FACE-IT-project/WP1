...
A Google Sheet was created to allow the experts at FACE-IT to fill-in which drivers were relevant for their work/study sites. The key drivers identified have been distributed into five categories presented below.
...
The use of the word 'Driver', rather than 'Variable', to describe data may seem strange to some. The argument for this decision is based on the fact that FACE-IT is a consortium of a many teams that employ a range of different methods of scientific inquiry. The technical vocabulary used across these groups may differ in some important ways so it was decided to defer to the definitions used in the IPCC. It must therefore be noted here that a 'Driver' does not necessarily mean that something is a physical (or otherwise) driver of another variable. In the context of FACE-IT most of the types of data that are being measured are indeed drivers of other types of data, but it is not necessary for them to be in order to be labelled as 'Drivers'. For some it may be easier to think of the data outlined here as 'Variables', rather than 'Drivers'.
...
> __Bathymetry__; __Current__: direction, location, volume; __Evaporation__; __Heatflux__: net, latent/sensible, long/shortwave radiation; __Light extinction coefficient (Kd)__; __Mixed layer depth (MLD)__; __Precipitation__; __River discharge__; __Salinity__; __Sea level pressure__; __Seawater temperature__: surface, mid, bottom; __Sedimentation rate__; __Suspended matter__: organic, mineral; __Wind__: direction, speed
...
> __CaCO~3~ saturation state__; __Dissolved inorganic carbon (DIC)__; __Dissolved organic carbon (DOC)__; __Dissolved organic nitrogen (DON)__; __Dissolved O~2~__; __Nutrients__: nitrate (NO3), nitrite (NO2), ammonium (NH4), phosphate (PO4), silicate (SiO4); __Partial pressure of CO~2~__; __pH__; __Total alkalinity__
