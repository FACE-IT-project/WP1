...
socat <- data.table::fread(file = paste0(pCloud_path, "FACE-IT_data/socat/SOCATv2020.tsv"), 
                           select = c("yr", "mon", "day", "longitude [dec.deg.E]", "latitude [dec.deg.N]", "pCO2water_SST_wet [uatm]"),
                           skip = 6362, nrows = Inf, fill = TRUE, nThread = 4)
...
* `r nrow(socat_arctic_60)`, or `r round(100*nrow(socat_arctic_60)/nrow(socat_all), digits = 1)`%, in the Arctic Ocean (North of 60°N). 
* `r nrow(socat_arctic_75)`, or `r round(100*nrow(socat_arctic_75)/nrow(socat_all), digits = 1)`%, in the Arctic Ocean (North of 75°N). 
...
* `r nrow(glodap_arctic_60)`, or `r round(100*nrow(glodap_arctic_60)/nrow(glodap_all), digits = 1)`%, in the Arctic Ocean (North of 60°N)
* `r nrow(glodap_arctic_75)`, or `r round(100*nrow(glodap_arctic_75)/nrow(glodap_all), digits = 1)`%, in the Arctic Ocean (North of 75°N). 