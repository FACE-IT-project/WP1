...
```{r socat, message=FALSE, warning=FALSE, echo = FALSE}
...
socat_all <- socat_all %>% 
  mutate(lon = case_when(lon > 180 ~ lon-360, TRUE ~ lon),
         ocean = case_when(lon >= -70 & lon <= 20 ~ "Atlantic",
                           ...
                           TRUE ~ "Pacific")) %>% 
  mutate(ocean = case_when(lat >= 75 ~ "Arctic",
                           lat <= -60 ~ "Southern",
                           TRUE ~ ocean))
...
# global counts
global_count_deep <- socat_all %>% 
  group_by(ocean) %>% 
  summarise(n = n(), .groups = "drop") %>% 
  mutate(depth_group = "deep")
global_count_shallow <- socat_all %>% 
  filter(bottomdepth <= 200) %>% 
  group_by(ocean) %>% 
  summarise(n = n(), .groups = "drop") %>% 
  mutate(depth_group = "shallow")
global_count <- rbind(global_count_deep, global_count_shallow)

# Calculate annual counts
annual_count_deep <- socat_all %>% 
  group_by(product, ocean, year) %>% 
  summarise(n = n(), .groups = "drop") %>% 
  mutate(depth_group = "deep")
annual_count_shallow <- socat_all %>% 
  filter(bottomdepth <= 200) %>% 
  group_by(product, ocean, year) %>% 
  summarise(n = n(), .groups = "drop") %>% 
  mutate(depth_group = "shallow")
...
annual_count %>% 
  ...
  labs(title = "Number of SOCAT data per year for different oceans", 
...
monthly_count_deep <- socat_all %>% 
  group_by(product, ocean, month) %>% 
  summarise(n = n(), .groups = "drop") %>% 
  mutate(depth_group = "deep")
monthly_count_shallow <- socat_all %>% 
  filter(bottomdepth <= 200) %>% 
  group_by(product, ocean, month) %>% 
  summarise(n = n(), .groups = "drop") %>% 
  mutate(depth_group = "shallow")
...
monthly_count %>% 
  ...
  scale_x_continuous(breaks = 1:12, labels = month.abb, expand = c(0, 0)) +
  labs(title = "Count of SOCAT data per month for different oceans",
       #subtitle = "Note that there is so little GLODAP data by comparison it is difficult to see",
       x = "Month", y = "count") +
  facet_wrap(~depth_group) +
  theme(legend.position = "bottom")
```
Among the `r sum(global_count$n)` SOCAT v2021 comprises `r ``


```{r glodap, message=FALSE, warning=FALSE, echo = FALSE}
# case_when to apply names to ocean regions
# Group by these to get counts
# Bar plot of comparisons with fill showing product
glodap_all <- glodap %>% 
...
# Calculate annual counts
annual_count_deep <- glodap_all %>% 
  group_by(product, ocean, year) %>% 
  summarise(n = n(), .groups = "drop") %>% 
  mutate(depth_group = "deep")
annual_count_shallow <- glodap_all %>% 
  filter(bottomdepth <= 200) %>% 
  group_by(product, ocean, year) %>% 
  summarise(n = n(), .groups = "drop") %>% 
  mutate(depth_group = "shallow")
...
annual_count %>% 
  ...
  geom_point(aes(shape = product)) +
  geom_line(aes(linetype = product)) +
  labs(title = "Number of glodap data per year for different oceans", 
       #subtitle = "Note there is so much more glodap data it is hard to see GLODAP", 
       x = "Year", y = "count") +
  facet_wrap(~depth_group) +
  theme(legend.position = "bottom")

# Calculate monthly counts
monthly_count_deep <- glodap_all %>% 
  group_by(product, ocean, month) %>% 
  summarise(n = n(), .groups = "drop") %>% 
  mutate(depth_group = "deep")
monthly_count_shallow <- glodap_all %>% 
  filter(bottomdepth <= 200) %>% 
  group_by(product, ocean, month) %>% 
  summarise(n = n(), .groups = "drop") %>% 
  mutate(depth_group = "shallow")
...
monthly_count %>% 
  ...
  scale_colour_brewer(palette = "Set1") +
  scale_x_continuous(breaks = 1:12, labels = month.abb, expand = c(0, 0)) +
  labs(title = "Count of glodap data per month for different oceans",
       #subtitle = "Note that there is so little GLODAP data by comparison it is difficult to see",
       x = "Month", y = "count") +
  facet_wrap(~depth_group) +
  theme(legend.position = "bottom")
