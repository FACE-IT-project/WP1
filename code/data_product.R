...
EU_epagr_population <- read.csv("~/pCloudDrive/FACE-IT_data/EU_arctic/production-of-pups-and-e.csv", sep = ";", dec = ",") %>%
  ...
  mutate(date_accessed = as.Date("2023-04-14"),
         ...
         variable = paste0(Species, " ", str_replace_all(tolower(name),"\\."," ")," [n]"),
         category = "bio",
         driver = "biomass",
         site = "east ice",
         type = "survey",
...
barents_polar_cod <- read.csv("~/pCloudDrive/FACE-IT_data/svalbard/biomass-of-polar-cod-in.csv", sep = ";", dec = ",") %>% 
  mutate(date_accessed = as.Date("2023-04-11"), 
         ...
         # nomsp = map(Species, latin_eng),
         variable = paste0(Species, " [10^6 kg]"),
         category = "bio",
         driver = "biomass",
         site = "barents sea", 
         type = "in situ",
...
barents_capelin_stock <- read.csv("~/pCloudDrive/FACE-IT_data/svalbard/capelin-stock-in-the-bar.csv",sep = ";" , dec = ",") %>% 
  ...
  mutate(date_accessed = as.Date("2023-04-12"), 
         ...
         # nomsp = map(Species, latin_eng),
         variable = paste0(Species, " ", str_replace(tolower(name),"\\."," ") ," [10^6 kg]"),
         category = "bio",
         driver = "biomass",
         site = "barents sea", 
         type = "in situ",
...
barents_golden_redfish_population <- read_delim("~/pCloudDrive/FACE-IT_data/svalbard/stock-of-golden-redfish.csv") %>% 
  ...
  mutate(date_accessed = as.Date("2023-04-12"), 
         ...
         # nomsp = map(Species, latin_eng),
         variable = paste0(Species, " ", tolower(name) ," [10^3 kg]"),
         category = "bio",
         driver = "biomass",
         site = "barents sea", 
         type = "in situ",
...
barents_young_herring_population <- read.csv("~/pCloudDrive/FACE-IT_data/svalbard/biomass-index-for-young.csv", sep = ";", dec = ",") %>% 
  ...
  mutate(date_accessed = as.Date("2023-04-13"),
         ...
         # nomsp = map(Species, latin_eng),
         variable = paste0(Species, " ", substr(str_replace_all(tolower(name),"\\."," "),2, 11)," [n]"),
         category = "bio",
         driver = "biomass",
         site = "barents sea",
         type = "in situ",
...
sval_walrus_population <- read_delim("~/pCloudDrive/FACE-IT_data/svalbard/walrus-population-in-sva.csv") %>% 
  ...
  mutate(date_accessed = as.Date("2023-04-13"), 
         ...
         variable = paste0(Species, " ", type, " [n]"),
         category = "bio",
         driver = "biomass",
         type = "survey", site = "svalbard", 
         date = as.Date(paste0(Category,"-12-31"))) %>% 
...
sval_nw_calanus_mm_population <- read.csv("~/pCloudDrive/FACE-IT_data/svalbard/average-biomass-of-zoopl.csv", sep = ";", dec = ",") %>%
  ...
  mutate(date_accessed = as.Date("2023-04-13"),
         ...
         Species = "Zooplankton",
         # nomsp = map(Species, latin_eng),
         variable = paste0(Species, " ", substr(stri_replace_last(tolower(name)," ", regex = "[.]"), 2, 10)," [g/m²]"),
         category = "bio",
         driver = "biomass",
...
# ice_4km_open_wide <- plyr::ddply(ice_4km_proc, c("site"), ice_open_water, .parallel = T)
...
# Combine results and shape to project standard
# ice_4km_open <- ice_4km_open_wide |> 
#   pivot_longer(event_n:open_days, names_to = "variable", values_to = "value") |> 
#   mutate(variable = case_when(variable == "event_n" ~ "open water events annually [n]",
#                               variable == "open_days" ~ "open water days annually [days]"),
#          date = as.Date(paste0(year,"-12-31"))) |> 
#   dplyr::select(date, variable, value, site)
...
# Test depths etc.
test1 <- clean_sea_ice |> distinct(depth)
...
# Test depths etc.
test1 <- clean_glacier |> distinct(depth)
...
FACE_IT_GRDC <- site_GRDC |> 
  ...
  filter(!is.na(site)) |> pivot_longer(`Q [m3/s]`, names_to = "variable") |> 
...
# Test depths etc.
test1 <- clean_runoff |> distinct(depth)
...
# Test depths etc.
test1 <- clean_nutrients |> distinct(depth)
...
# Test depths etc.
test1 <- clean_prim_prod |> distinct(depth)
...
clean_spp_rich <- filter(full_ALL, driver == "biomass") |> 
  ...
  filter(!is.na(variable)) %>% arrange(variable) |> distinct() |> depth_bin_average() |>  
  summarise(value = mean(value, na.rm = TRUE), 
            .by = c(date_accessed, URL, citation, type, site, category, driver, variable, lon, lat, date, depth))
...
# Test depths etc.
test1 <- clean_gov |> distinct(depth)
...
# Test depths etc.
test1 <- clean_tourism |> distinct(depth)
...
# Test depths etc.
test1 <- clean_fisheries |> distinct(depth)
...
save(clean_all, file = "data/full_data/clean_all.RData")
